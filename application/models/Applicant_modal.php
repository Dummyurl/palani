...
class Applicant_modal extends CI_Model
{
    ...
    {   
        ...
        LEFT JOIN social_applicant_user s ON s.reference_id = p.mentor_id WHERE a.role = '1' AND p.user_id = '$user_id' ";
...
{
    $this->db->select('t1.*,t2.id as app_id,t2.first_name,t2.last_name,t2.username,t2.profile_img,t2.logged_in,t3.*,t4.picture_url');
    $this->db->from('invite t1');
    $this->db->where('t1.invite_from',$id);
    $this->db->where('t1.approved',1);
    $this->db->where('t1.current_status',0);
