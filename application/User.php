...
function generate_username($string_name="", $rand_no = 200){
      ...
      Public function get_all_payments()
      {
        ...
        foreach ($list as $g) {
         ...
         $row[] = date('h:i:s A',strtotime($g->payment_date));   
   ...
   {                          
     ...
     $this->email->message($message);
             ...
             {
              $applicant_id     = $this->session->userdata('applicant_id'); 
              $this->db->update('applicants',array('logged_in'=>0),array('id'=>$applicant_id ));
...
public function get_messages()
{
  ...
  if(count($latest_chat)>4){
    $html ='<div align="center" class="pre"><button class="load_more btn btn-xs btn-primary" align="center">Load old messages . .</button></div>';    
  }else{
    $html ='';
  }
...
public function send_invite_to_applicant()
{
  ...
  $notify_data['user_id'] = $invitedata['invite_to'];
  $notify_data['notification_id'] = $this->session->userdata('applicant_id');
  $notify_data['text'] = $guru['first_name'].' '.$guru['last_name'].' invited you';
  $notify_data['read'] = 0;
  ...
  $this->email->from('dinesh@dreamguys.co.in','SchoolGuru');
  $this->email->to($data['result']['email']);
  $this->email->subject('Hey! '.$data['result']['first_name'].' You have been invited from SchoolGuru ! ');
  $this->email->message($message);
                if($this->email->send()) //mail Function*/  
                {
                  $sts = 0;                 
...
public function delete_conversation()
{
  ...
  $data = $this->applicant_modal->get_latest_chat($selected_user,$session_id);
  if(!empty($data)){
    foreach ($data as $d) {
  ...
  // $cond.="( `sent_id` = '". $sender_id ."' and `recieved_id` = " . $id . ") OR ";
  // $cond_1.="( `sent_id` = '". $id ."' and `recieved_id` = " . $sender_id . ")";
  // $this->db->where($cond.$cond_1);
  // $update = $this->db->update('chat',array('delete_sts'=>1));
  // echo $update;
}
...
if(!empty($da)){
  ...
  $rangeArray = array_diff($rangeArray,$old_datas);
}  

$data['available'] = json_encode($rangeArray);
...
Public function approve_activity()
{
  ...
  error_reporting(1);
  $this->db->update('invite',array('approved'=>1),array('invite_id'=>$_POST['invite_id']));
...
Public function get_call()
{
  ...
  if(isset($result->status)==1){
    ...
    <a class="btn btn-success join" href="'.base_url().'user/incoming_video_call/'.$username.'/'.$channel.'" call_id="'.$result->call_id.'">Join </a>
...
{
  $where = array('call_id',$_POST['call_id']);
