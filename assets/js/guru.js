            ...
            success: function(data)
            {
                                    ...
                                    if(data){
                                        ...
                                        $(obj.gurus).each(function(){
                                            names +='<a href="'+base_url+'search-mentor/'+keyword+'">'+this.mentor_personal_message+'</a>';
                                        });
...
// var mobile_verified = $('#mobile_verified').text();
// if((verify == 0 && mobile_verified == 0))
// {
...
//                                                else
//                                                {
//                                                 $('#error_msg').html("Mobile number already registerd ! try other number to send OTP");
...
//                  '<p>'+obj.data.file_name+'</p>'+
//                  '<a href="'+file_name+'" target="_blank" download class="chat-time">Download</a>'+
//                  '<span class="chat-time">'+time+'</span>'+
//                  '</div>'+
//                  '</div>'+
//                  '</div>';
    ...
    $('.callbox').each(function(){
        ...
        var applicant_username = $('#applicant_username'+i).val();
                    ...
                    if(days>0){ // More than Today
                       ...
                       $('.conversation_start'+invite_id).html('<ul>'+
                        '<li><a href="'+base_url+'user/incoming_video_call/'+username+'/'+channel+'/'+applicant_id+'/'+encoded_invite_id+'" class="conv_videocall startVideo" userid="'+applicant_user_id+'"><i aria-hidden="true" class="fa fa-video-camera"></i></a></li>'+
                        '<li><a href="'+base_url+'user/incoming_audio_call/'+username+'/'+channel+'/'+applicant_id+'/'+encoded_invite_id+'" class="conv_audiocall startAudio" userid="'+applicant_user_id+'"><i aria-hidden="true" class="fa fa-phone"></i></a></li>'+
                        '<li><a href="#" class="conv_messages"  data-chat-id="'+applicant_user_id+'" data-username="'+applicant_username+'" data-name="'+applicant_name+'" data-image="'+applicant_image+'" data-status="'+log_class+'"><i aria-hidden="true" class="fa fa-comments-o"></i></a></li>'+
                        '</ul>'+
...
function checkinner_validation()
{
   ...
   if(search_value=='')
   {
     ...
     swal("Warning!",'Please enter text to search', "error");
     return false;
 }
