...
class Crop_call { 
    ...
    public function cropper_initialize(){
 		
 		$path = FCPATH.'assets/images/crop/';
    	...
    	$this->output_css_contents = $this->get_cropper_min_css();
        $this->output_js_contents  = $this->get_cropper_min_js();
		$this->output_html 		   = $this->get_cropper_html();
		$this->output_popup_html    = $this->get_cropper_popup_html();
		return array('css' => $this->output_css_contents,'html' => $this->output_html,'popup_html'=>$this->output_popup_html,'js' => $this->output_js_contents);
    }


    public function get_cropper_html(){
    	$html_content =  '<div class="crop-image"><div class="img-cropper"><div class="crop-area"><div class="crop-result"></div></div><div class="crop-action"></div></div><div class="upload-btn"><a href="javascript:void(0);"  class="btn btn-primary upload_image_btn" onclick="openfile()">Upload Your Image</a></div><div class="result-details"></div><h4>Crop your images with a perfect balance in quality and file size.</h4></div>';

    	return $html_content;
    }
    
    public function get_cropper_popup_html(){

    	$html_popup_content = '<div id="crop-avatar">
				<div class="modal fade avatar-open" id="avatar-modal" aria-hidden="true" aria-labelledby="avatar-modal-label" style="top:50px;" role="dialog" tabindex="-1">
					<div class="modal-dialog modal-lg">
						<div class="modal-content">
							<form class="avatar-form" action="'.base_url().'crop/library_call" enctype="multipart/form-data" method="post">
								...
								<div class="modal-body">
									<div class="avatar-body">
										...
										<div class="avatar-upload">
											...
											<label for="avatarInput">Select upload image</label>
											<input type="file"  accept="image/*" class="avatar-input ad_pd_file" id="avatarInput" name="avatar_file"  />
										</div>
										<div class="row" style="margin-top:20px;">
											<div class="col-md-12">
												<div class="avatar-wrapper text-center"></div>
											</div>
										</div>
    ...
    public function get_cropper_min_css(){
		...
		.loading {display: none;position: absolute;top: 0;right: 0;bottom: 0;left: 0;background: #fff url("'.base_url().'/assets/images/loader.gif") no-repeat center center;opacity: .75;filter: alpha(opacity=75);z-index: 20140628;}
		...
		.img-cropper {background: #e0e0e0 url("'.base_url().'/assets/images/img-icon.png") no-repeat scroll center center;border-radius: 10px;margin: 0 auto 40px;position: relative;width: 250px;height: 250px;}
		...
		.crop-delete {background-image: url("'.base_url().'/assets/images/delete.png");    background-color: rgba(0, 0, 0, 0.7);    background-position: 50% center;    background-repeat: no-repeat;    background-size: 50% 50%;    border: medium none;    border-radius: 50%;    color: #fff;    font-size: 0;    height: 36px;    outline: medium none;position: relative;width: 36px;cursor:pointer;display:inline-block;margin:0 2px;}
		.crop-download{background-image: url("'.base_url().'/assets/images/download.png");background-color: rgba(0, 0, 0, 0.7);background-position: 50% center;background-repeat: no-repeat;background-size: 50% 50%;border: medium none;border-radius: 50%;color: #fff;font-size: 0;height: 36px;outline: medium none;position: relative;width: 36px;cursor:pointer;display:inline-block;margin:0 2px;}
		.crop-edit{background-image: url("'.base_url().'/assets/images/edit.png");background-color: rgba(0, 0, 0, 0.7);background-position: 50% center;background-repeat: o-repeat;background-size: 50% 50%;border: medium none;border-radius: 50%;color: #fff;font-size: 0;height: 36px;outline: medium none;position: relative;width: 36px;cursor:pointer;display:inline-block;margin:0 2px;}
    ...
    public function get_cropper_min_js(){
		...
		// Cropper Min js Endhere 

		 $js_content .= '(function(factory) {
    ...
    $js_content .= 'var file_string ="';
    $js_content .= '<input type='."'file'".'>';
    $js_content .= '";';
    $single_quote = "'";
    $double_quote = '"';

    $image_file1  = "'<img src=".$double_quote. "'";
    $image_file2  =  "'".$double_quote.">'";
    ...
    $js_content .= 'function CropAvatar($element) {
    ...
    CropAvatar.prototype = {
        ...
        support: {
            fileList: !!$(file_string).prop("files"),
...
$js_content .= 'initPreview: function() {
            var url = this.$avatar.attr("src");
            this.$avatarPreview.html('.$image_file1.' + url + '.$image_file2.');
        },
...
$js_content .= '$iframe.one("load", function() {
    ...
    $js_content .= 'change: function() {
        ...
        startCropper: function() {
            ...
            if (this.active) {
                ...
                this.$img = $('.$image_file1.'+ this.url + '.$image_file2.');
        ...
        $js_content .= '$.ajax(url, {
                ...
                contentType: false,
                beforeSend: function() {';
                $curent_string = "<img width='24' height='24' src='".base_url()."assets/images/btn-loader.svg' alt='loading'>";
                $js_content .= ' $(".avatar-save").html("';
                $js_content .= $curent_string.'");
        ...
        submitDone: function(data) {
            if (data.success == "Y") {
               ...
               $delete_img = '<a class="crop-delete" id="delete_img" title="Delete" type="button" onclick="deleteimage(';
               $delete_img .= "\'";
               $delete_img .= "'+ data.cropped_fliepath+'"; 
           	   $delete_img .= "\'";
           	   $delete_img .= ')">Delete</a><a href="';
               $delete_img .= "'+BASE_URL + data.cropped_fliepath+'".$double_quote; 
               $delete_img .= ' class="crop-download" download  title="Download">Download</a>';

               

            $my_img = '<img width="250" class="upload_image_btn" height="250" src="';
            $my_img .= "'+BASE_URL + data.cropped_fliepath+'";
            $my_img .= '" alt="loading">';

            $my_img = "'".$my_img."'";
            $delete_img = "'".$delete_img."'";

            $js_content .= '$(".crop-result").html ('.$my_img.');
                ...
                $(".crop-action").html('.$delete_img.');
                var htmltable = ';

             $mydata =  "<div class='table-responsive'><table class='table table-striped table-bordered result-table'><thead><tr><th>Property</th><th>Value</th></tr></thead><tbody><tr><td class='Bold'>Name</td><td><p>".$double_quote."+data.full_fliename +".$double_quote."</p></td></tr><tr><td class='Bold'>Type</td><td><p>".$double_quote."+ data.image_extension +".$double_quote."</p></td></tr><tr><td class='Bold'>Date</td><td><p>".$double_quote."+ data.Date +".$double_quote."</p></td></tr><tr><td class='Bold'>File path</td><td><p>".$double_quote."+data.cropped_fliepath +".$double_quote."</p></td></tr><tr><td class='Bold'>Success</td><td><p>".$double_quote."+data.success +".$double_quote."</p></td></tr></tbody></table></div>";
             
             $js_content .= '"'.$mydata.'";';

             $js_content .='$(".result-details").html(htmltable);
        ...
        alert: function(msg) {
            var $alert = [';	
            $string_1 = '<div class="alert alert-danger avatar-alert alert-dismissable">';
            $string_1 = "'".$string_1."'";
            
            $string_2 = '<button type="button" class="close" data-dismiss="alert">&times;</button>';
			$string_2 =  "'".$string_2."'";
            $js_content .= $string_1.",".$string_2.',';
            $js_content .= 'msg,
                "</div>"
            ].join("");
...
function deleteimage(videolink) {
    ...
    var url = BASE_URL + "crop/imageunlink";
    $.ajax({
        type: "POST",
        url: url,
        data: {
...
function openfile() {
	...
	public function prf_crop($av_src,$av_data,$av_file) {	
					$directoryName ='assets/images/crop/';
					...
					$new_name1       = $file_name.time()."_250x250.".$imageFileType; 
					$image1          = $this->prf_crop_call($image_name,$av_data,$new_name1,$directoryName,250,250);
					$cropfliename     = $directoryName.$new_name1;
					$data['success'] = 'Y';
					}else{
							$data['success'] = 'n';
					}
					...
					$data['cropped_fliepath'] =$cropfliename;
					return $data;  
					
				}
	public function prf_crop_call($image_name,$av_data,$new_name,$directoryName,$t_width,$t_height) {  
   			$w                 = $av_data['width'];
			$h                 = $av_data['height'];
			$x1                = $av_data['x'];
			$y1                = $av_data['y'];
