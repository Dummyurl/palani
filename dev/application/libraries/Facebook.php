...
Class Facebook
{
    ...
    public function __construct(){
        ...
        $this->load->config('facebook'); // Commented by navaneeth 19-07-2017        
        $this->config->config['facebook_login_redirect_url']=  strtolower('applicant')."_fb_login/user_authentication";        
     ...
     * @return  string
     */
    public function signup_applicant_url(){
        // Login type must be web, else return empty string
        if($this->config->item('facebook_login_type') != 'web'){
            return '';
        }
        // Get login url
        return $this->helper->getLoginUrl(
            base_url() . $this->config->item('facebook_signup_applicant_redirect_url'),
            $this->config->item('facebook_permissions')
        );
    }
     ...
     * @return  string
     */
    public function signup_guru_url(){
        // Login type must be web, else return empty string
        if($this->config->item('facebook_login_type') != 'web'){
            return '';
        }
        // Get login url
        return $this->helper->getLoginUrl(
            base_url() . $this->config->item('facebook_signup_guru_redirect_url'),
            $this->config->item('facebook_permissions')
        );
    }
