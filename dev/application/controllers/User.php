         ...
         $user_name = trim($_POST['user_name']);
          ...
          if($this->session->userdata('role')==0){ // Applicant 
            ...
            // LIMIT 5";
          	...
          	$sql = "SELECT a.first_name,a.last_name, a.profile_img, s.picture_url, a.username, m.mentor_job_title,m.mentor_languages_speak,m.mentor_school,m.mentor_job_company,`m`.mentor_personal_message,c.city,st.statename,cy.country   FROM applicants a 
            LEFT JOIN `social_applicant_user` `s` ON `s`.`reference_id` = `a`.`id`
            LEFT  JOIN `mentor_details` `m` ON `m`.`mentor_id` = `a`.`id` 
...
public function process(){      
	try {
		...
		$chargeArr = array(
            ...
            // echo '<pre>'; print_r($chargeArr); exit;
...
function add_schedule_time()
{
 ...
 $rangeArray=array();
  ...
  if(!empty($da)){
  	...
  	$rangeArray = array_values(array_diff($rangeArray,$old_datas));
  }  
