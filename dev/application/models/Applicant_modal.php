...
class Applicant_modal extends CI_Model
{
    ...
    {   
        ...
        $payment_date = base64_decode($this->uri->segment(4));
          ...
          $sql = "SELECT a.*,p.payment_date,p.payment_id,p.payment_status,SUM(payment_gross) as earned,COUNT(p.payment_id)as calls,a.delete_sts, a.username as guru_user_name,a.profile_img ,CONCAT(a.first_name,' ',a.last_name) AS guru_name,s.picture_url FROM applicants a LEFT JOIN payments p ON a.id = p.mentor_id LEFT JOIN social_applicant_user s ON s.reference_id = p.mentor_id WHERE mentor_id ='$mentor_id' AND payment_date = '$payment_date' GROUP BY p.payment_date";
    ...
    {   
        ...
        $sql ="SELECT p.payment_date,p.mentor_id,p.payment_id,p.payment_status,SUM(payment_gross) as earned,COUNT(p.payment_id)as calls,a.delete_sts, a.username as applicant_user_name,a.profile_img as applicant_profile_img,CONCAT(a.first_name,' ',a.last_name) AS applicant_name,s.picture_url as applicant_picture FROM applicants a LEFT JOIN payments p ON a.id = p.mentor_id LEFT JOIN social_applicant_user s ON s.reference_id = p.mentor_id WHERE a.role = '1' AND p.user_id = '$user_id' ";
    ...
    public function check_applicant_login($params)
    {
        ...
        if(sizeof($query)>0)
        {
            $this->session->set_userdata(array('applicant_id'=>$query['id'],'role'=>$query['role'],'type'=>$query['type']));
...
WHERE applicants.id=$applicant_id";
    ...
    public function mentor_list_view()
    {
        $query = $this->db->query("SELECT applicants.id as app_id,applicants.first_name,applicants.last_name,applicants.username,applicants.email,applicants.profile_img,applicants.is_verified,country_list.country_name,country_list.code,social_applicant_user.picture_url,(select COUNT(rating) from review_ratings where user_id=applicants.id) as rating_count,(select ROUND(AVG(rating)) from review_ratings where user_id=applicants.id) as rating_value,mentor_details.* FROM applicants 
            ...
            LEFT JOIN `social_applicant_user` ON social_applicant_user.reference_id = applicants.id
            LEFT JOIN country_list ON country_list.country_id = mentor_details.country where applicants.is_verified=1 and applicants.profile_updated=1 and applicants.role=1 LIMIT 0,5")->result_array();        
    ...
    public function mentor_list_view_count()
    {
        $query = $this->db->query("SELECT applicants.id as app_id,applicants.first_name,applicants.last_name,applicants.username,applicants.email,applicants.profile_img,applicants.is_verified,country_list.country_name,country_list.code,social_applicant_user.picture_url,mentor_details.* FROM applicants 
            LEFT JOIN mentor_details ON mentor_details.mentor_id = applicants.id
            LEFT JOIN `social_applicant_user` ON social_applicant_user.reference_id = applicants.id
            LEFT JOIN country_list ON country_list.country_id = mentor_details.country where applicants.is_verified=1 and applicants.profile_updated=1 and applicants.role=1")->result_array();        
        return count($query);
    }

    public function applicant_detail_list_view($id)
    {
        $query = $this->db->query("SELECT applicants.id as app_id,applicants.id,applicants.first_name,applicants.username,applicants.last_name,applicants.email,applicants.profile_img,applicants.is_verified,country_list.country_name,country_list.code,social_applicant_user.picture_url,(select COUNT(rating) from review_ratings where user_id=applicants.id) as rating_count,(select ROUND(AVG(rating)) from review_ratings where user_id=applicants.id) as rating_value,mentor_details.* FROM applicants 
            LEFT JOIN mentor_details ON mentor_details.mentor_id = applicants.id
            LEFT JOIN `social_applicant_user` ON social_applicant_user.reference_id = applicants.id
            LEFT JOIN country_list ON country_list.country_id = mentor_details.country where applicants.is_verified=1 and applicants.profile_updated=1 and applicants.id=$id")->row_array();        
    ...
    public function update_profile($applicant_id,$profile_details)
    {
        ...
        {               
            ...
            {   
                ...
                if($this->db->update('applicants_profile',$profile_details))
                {
                    $sts = 0 ;
                }
            }
            ...
            {                
                if($this->db->insert('applicants_profile',$profile_details))
                {
                    $sts = 0 ;
                }                
            }
...
public function get_latest_chat($selected_user,$session_id)
{
   ...
   $this->update_counts($selected_user,$session_id);
    ...
    //  echo "SELECT DISTINCT CONCAT(sender.first_name,' ',sender.last_name) as senderName, sender.profile_img as senderImage, sender.id as sender_id,msg.msg, msg.chatdate,social.picture_url as socialImage,msg.id,msg.type,msg.file_name,msg.file_path,time_zone,msg.id
    // from chat msg  
    // LEFT  join applicants sender on msg.sent_id = sender.id
    // LEFT  join social_applicant_user social on social.reference_id = msg.sent_id
    // left join chat_deleted_details cd on cd.chat_id  = msg.id
...
public function search_mentor_message($keyword)
{
    if($keyword != '')
    {
     ...
     $query = $this->db->query("SELECT mentor_personal_message from mentor_details WHERE  $mentor_personal_message group by mentor_personal_message LIMIT 0,5")->result_array(); 
     return $query;
 }
}
...
public function search_mentor_list_view($keyword)
{
    if($keyword != '')
    {
    $mentor_personal_message = "(`applicants`.`first_name` LIKE '%$keyword%' ESCAPE '!' 
            OR `applicants`.`last_name` LIKE '%$keyword%'
            OR `mentor_details`.mentor_school LIKE '%$keyword%' 
            OR `mentor_details`.mentor_languages_speak LIKE '%$keyword%' 
            OR `mentor_details`.mentor_job_company LIKE '%$keyword%'
            OR `mentor_details`.mentor_job_title LIKE '%$keyword%'
            OR `mentor_details`.mentor_personal_message LIKE '%$keyword%'
            )";
     ...
     $query = $this->db->query("SELECT applicants.id as app_id,applicants.first_name,applicants.last_name,applicants.username,applicants.email,applicants.profile_img,applicants.is_verified,country_list.country_name,country_list.code,social_applicant_user.picture_url,(select COUNT(rating) from review_ratings where user_id=applicants.id) as rating_count,(select ROUND(AVG(rating)) from review_ratings where user_id=applicants.id) as rating_value,mentor_details.* FROM applicants 
        ...
        $mentor_personal_message group by applicants.id LIMIT 0,5")->result_array(); 
...
public function search_mentor_list_view_count($keyword)
{
 ...
 $query = $this->db->query("SELECT applicants.id as app_id,applicants.first_name,applicants.last_name,applicants.username,applicants.email,applicants.profile_img,applicants.is_verified,country_list.country_name,country_list.code,social_applicant_user.picture_url,mentor_details.* FROM applicants 
    ...
    LEFT JOIN `social_applicant_user` ON social_applicant_user.reference_id = applicants.id 
    LEFT JOIN country_list ON country_list.country_id = mentor_details.country where applicants.role=1 and applicants.is_verified=1 and applicants.profile_updated=1 and 
    $mentor_personal_message")->result_array();        
 return count($query);
}
